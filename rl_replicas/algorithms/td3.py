...
class TD3(OffPolicyAlgorithm):
  ...
  def train(
    ...
    for train_step in range(train_steps):
      minibatch: Dict[str, torch.Tensor] = replay_buffer.sample_minibatch(minibatch_size)
      ...
      with torch.no_grad():
        ...
        epsilon: torch.Tensor = self.target_noise_scale * torch.randn_like(next_actions)
        ...
        target_q_values_1: torch.Tensor = self.target_q_function_1(next_observations, next_actions)
        target_q_values_2: torch.Tensor = self.target_q_function_2(next_observations, next_actions)
        target_q_values: torch.Tensor = torch.min(target_q_values_1, target_q_values_2)
      ...
      if train_step % self.policy_delay == 0:
        ...
        policy_q_values: torch.Tensor = self.q_function_1(observations, policy_actions)
    ...
    logger.info('Q Function Loss (1):    {:<8.3g}'.format(np.mean(q_function_1_losses)))
    logger.info('Q Function Loss (2):    {:<8.3g}'.format(np.mean(q_function_2_losses)))
