...
class TRPO():
  ...
  def learn(
    ...
    old_policy: Policy = copy.deepcopy(self.policy)
    ...
    for current_epoch in range(epochs):
      ...
      def compute_surrogate_loss() -> torch.Tensor:
        ...
        with torch.no_grad():
          old_policy_dist: Categorical = old_policy(observations_tensor)
          old_log_probs: torch.Tensor = old_policy_dist.log_prob(actions_tensor)

        log_prob_ratio: torch.Tensor = (log_probs - old_log_probs).exp()
        surrogate_loss: torch.Tensor = -(log_prob_ratio * advantages_tensor).mean()
